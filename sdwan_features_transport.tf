...
resource "sdwan_transport_management_vpn_interface_ethernet_feature" "transport_management_vpn_interface_ethernet_feature" {
  ...
  ipv4_address_type                   = try(each.value.interface.ipv4_configuration_type, local.defaults.sdwan.feature_profiles.transport_profiles.management_vpn.ethernet_interfaces.ipv4_configuration_type)
...
resource "sdwan_transport_routing_ospf_feature" "transport_routing_ospf_feature" {
  for_each = {
  ...
  route_policy_id                                    = try(sdwan_transport_route_policy_feature.transport_route_policy_feature["${each.value.profile.name}-${each.value.ospf.route_policy}"].id, null)
...
resource "sdwan_transport_wan_vpn_feature_associate_routing_bgp_feature" "transport_wan_vpn_feature_associate_routing_bgp_feature" {
  for_each = {
    for profile in try(local.feature_profiles.transport_profiles, {}) :
    "${profile.name}-wan_vpn-routing_bgp" => profile
...
resource "sdwan_transport_wan_vpn_feature_associate_routing_ospf_feature" "transport_wan_vpn_feature_associate_routing_ospf_feature" {
  for_each = {
    for profile in try(local.feature_profiles.transport_profiles, {}) :
    "${profile.name}-wan_vpn-routing_ospf" => profile
    if try(profile.wan_vpn.ospf, null) != null
  }
  feature_profile_id                = sdwan_transport_feature_profile.transport_feature_profile[each.value.name].id
  transport_wan_vpn_feature_id      = sdwan_transport_wan_vpn_feature.transport_wan_vpn_feature["${each.value.name}-wan_vpn"].id
...
resource "sdwan_transport_wan_vpn_interface_ethernet_feature" "transport_wan_vpn_interface_ethernet_feature" {
  ...
  ipv4_address                   = try(each.value.interface.ipv4_address, null)
  ipv4_address_variable          = try("{{${each.value.interface.ipv4_address_variable}}}", null)
  ipv4_address_type              = try(each.value.interface.ipv4_configuration_type, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ethernet_interfaces.ipv4_configuration_type)
  ipv4_dhcp_distance             = try(each.value.interface.ipv4_dhcp_distance, null)
  ipv4_dhcp_distance_variable    = try("{{${each.value.interface.ipv4_dhcp_distance_variable}}}", null)
  ipv4_dhcp_helper               = try(each.value.interface.ipv4_dhcp_helpers, null)
  ipv4_dhcp_helper_variable      = try("{{${each.value.interface.ipv4_dhcp_helpers_variable}}}", null)
...
resource "sdwan_transport_wan_vpn_interface_ipsec_feature" "transport_wan_vpn_interface_ipsec_feature" {
  ...
  ipv4_address                             = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" ? try(each.value.interface.ipv4_address, null) : null
  ipv4_address_variable                    = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" ? try("{{${each.value.interface.ipv4_address_variable}}}", null) : null
  ipv4_mtu                                 = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" ? try(each.value.interface.ipv4_mtu, null) : null
  ipv4_mtu_variable                        = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" ? try("{{${each.value.interface.ipv4_mtu_variable}}}", null) : null
  ipv4_subnet_mask                         = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" ? try(each.value.interface.ipv4_subnet_mask, null) : null
  ipv4_subnet_mask_variable                = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" ? try("{{${each.value.interface.ipv4_subnet_mask_variable}}}", null) : null
  ipv4_tcp_mss                             = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" ? try(each.value.interface.ipv4_tcp_mss, null) : null
  ipv4_tcp_mss_variable                    = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" ? try("{{${each.value.interface.ipv4_tcp_mss_variable}}}", null) : null
  ipv6_address                             = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay" ? try(each.value.interface.ipv6_address, null) : null
  ipv6_address_variable                    = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay" ? try("{{${each.value.interface.ipv6_address_variable}}}", null) : null
  ipv6_mtu                                 = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay" ? try(each.value.interface.ipv6_mtu, null) : null
  ipv6_mtu_variable                        = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay" ? try("{{${each.value.interface.ipv6_mtu_variable}}}", null) : null
  ipv6_tcp_mss                             = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay" ? try(each.value.interface.ipv6_tcp_mss, null) : null
  ipv6_tcp_mss_variable                    = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay" ? try("{{${each.value.interface.ipv6_tcp_mss_variable}}}", null) : null
  ...
  tunnel_destination_ipv4_address          = (try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay") ? try(each.value.interface.tunnel_destination_ipv4_address, null) : null
  tunnel_destination_ipv4_address_variable = (try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay") ? try("{{${each.value.interface.tunnel_destination_ipv4_address_variable}}}", null) : null
  tunnel_destination_ipv6_address          = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" ? try(each.value.interface.tunnel_destination_ipv6_address, null) : null
  tunnel_destination_ipv6_address_variable = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" ? try("{{${each.value.interface.tunnel_destination_ipv6_address_variable}}}", null) : null
  ...
  tunnel_source_ipv4_address               = (try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay") ? try(each.value.interface.tunnel_source_ipv4_address, null) : null
  tunnel_source_ipv4_address_variable      = (try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4" || try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv4-v6overlay") ? try("{{${each.value.interface.tunnel_source_ipv4_address_variable}}}", null) : null
  tunnel_source_ipv6_address               = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" ? try(each.value.interface.tunnel_source_ipv6_address, null) : null
  tunnel_source_ipv6_address_variable      = try(each.value.interface.tunnel_mode, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.ipsec_interfaces.tunnel_mode) == "ipv6" ? try("{{${each.value.interface.tunnel_source_ipv6_address_variable}}}", null) : null
}
