...
resource "sdwan_transport_cellular_profile_feature" "transport_cellular_profile_feature" {
  ...
  slice_type                        = try(each.value.cellular.slice_type, null)
  slice_type_variable               = try("{{${each.value.cellular.slice_type_variable}}}", null)
  slice_differentiator              = try(each.value.cellular.slice_differentiator, null)
  slice_differentiator_variable     = try("{{${each.value.cellular.slice_differentiator_variable}}}", null)
}
...
resource "sdwan_transport_management_vpn_interface_ethernet_feature" "transport_management_vpn_interface_ethernet_feature" {
  ...
  enable_dhcpv6                       = try(each.value.interface.ipv6_address_type, null) == "dynamic" ? true : null
  ...
  ipv4_address_type                   = try(each.value.interface.ipv4_address_type, null)
  ipv4_address_type_variable          = try("{{${each.value.interface.ipv4_address_type_variable}}}", null)
  ...
  ipv6_address_type          = try(each.value.interface.ipv6_address_type, null)
  ipv6_address_type_variable = try("{{${each.value.interface.ipv6_address_type_variable}}}", null)
...
resource "sdwan_transport_wan_vpn_interface_cellular_feature" "transport_wan_vpn_interface_cellular_feature" {
  for_each = {
    for interface_item in flatten([
      for profile in try(local.feature_profiles.transport_profiles, {}) : [
        for wan_vpn in try([profile.wan_vpn], []) : [
          for interface in try(wan_vpn.cellular_interfaces, []) : {
            profile   = profile
            wan_vpn   = wan_vpn
            interface = interface
          }
  ...
  enable_ipv6                              = try(each.value.interface.enable_ipv6, null)
  enable_ipv6_variable                     = try("{{${each.value.interface.enable_ipv6_variable}}}", null)
  interface_description                    = try(each.value.interface.interface_description, null)
  interface_description_variable           = try("{{${each.value.interface.interface_description_variable}}}", null)
  interface_mtu                            = try(each.value.interface.interface_mtu, null)
  interface_mtu_variable                   = try("{{${each.value.interface.interface_mtu_variable}}}", null)
  ...
  tunnel_interface_encapsulations = try(each.value.interface.tunnel_interface == null, true) ? null : flatten([
    try(each.value.interface.tunnel_interface.ipsec_encapsulation, local.defaults.sdwan.feature_profiles.transport_profiles.wan_vpn.cellular_interfaces.tunnel_interface.ipsec_encapsulation) ? [{
      encapsulation       = "ipsec"
  ...
  tunnel_interface_color_description                      = try(each.value.interface.tunnel_interface.color_description, null)
  tunnel_interface_color_description_variable             = try("{{${each.value.interface.tunnel_interface.color_description_variable}}}", null)
  ...
  tunnel_interface_full_port_hop                          = try(each.value.interface.tunnel_interface.full_port_hop, null)
  tunnel_interface_full_port_hop_variable                 = try("{{${each.value.interface.tunnel_interface.full_port_hop_variable}}}", null)
...
resource "sdwan_transport_wan_vpn_interface_cellular_feature_associate_tracker_feature" "transport_wan_vpn_interface_cellular_feature_associate_tracker_feature" {
  for_each = {
    for interface_item in flatten([
      for profile in try(local.feature_profiles.transport_profiles, {}) : [
        for wan_vpn in try([profile.wan_vpn], []) : [
  ...
  feature_profile_id                              = sdwan_transport_feature_profile.transport_feature_profile[each.value.profile.name].id
  transport_wan_vpn_feature_id                    = sdwan_transport_wan_vpn_feature.transport_wan_vpn_feature["${each.value.profile.name}-wan_vpn"].id
  transport_wan_vpn_interface_cellular_feature_id = sdwan_transport_wan_vpn_interface_cellular_feature.transport_wan_vpn_interface_cellular_feature["${each.value.profile.name}-wan_vpn-${each.value.interface.name}"].id
...
resource "sdwan_transport_wan_vpn_interface_cellular_feature_associate_tracker_group_feature" "transport_wan_vpn_interface_cellular_feature_associate_tracker_group_feature" {
  for_each = {
    for interface_item in flatten([
      for profile in try(local.feature_profiles.transport_profiles, {}) : [
        for wan_vpn in try([profile.wan_vpn], []) : [
...
resource "sdwan_transport_wan_vpn_interface_ethernet_feature" "transport_wan_vpn_interface_ethernet_feature" {
  ...
  duplex_variable                      = try("{{${each.value.interface.duplex_variable}}}", null)
  enable_dhcpv6                        = try(each.value.interface.ipv6_address_type, null) == "dynamic" ? true : null
  enable_enforced_propagation          = try(each.value.interface.trustsec.enable_enforced_propagation, null)
  enable_ha_interlink_interface        = try(each.value.interface.enable_ha_interlink_interface, null)
  enable_sgt_propagation               = try(each.value.interface.trustsec.enable_sgt_propagation, null)
  enforced_security_group_tag          = try(each.value.interface.trustsec.enforced_security_group_tag, null)
  enforced_security_group_tag_variable = try("{{${each.value.interface.trustsec.enforced_security_group_tag_variable}}}", null)
  ...
  ipv6_address_type          = try(each.value.interface.ipv6_address_type, null)
  ipv6_address_type_variable = try("{{${each.value.interface.ipv6_address_type_variable}}}", null)
  ipv6_dhcp_secondary_address = try(length(each.value.interface.ipv6_dynamic_secondary_addresses) > 0, false) ? [for a in each.value.interface.ipv6_dynamic_secondary_addresses : {
    address          = try(a.address, null)
    address_variable = try("{{${a.address_variable}}}", null)
  }] : null
  ipv6_secondary_addresses = try(length(each.value.interface.ipv6_static_secondary_addresses) > 0, false) ? [for a in each.value.interface.ipv6_static_secondary_addresses : {
    address          = try(a.address, null)
    address_variable = try("{{${a.address_variable}}}", null)
  }] : null
  load_interval               = try(each.value.interface.load_interval, null)
  ...
  propagate                                  = try(each.value.interface.trustsec.propagate, null)
  qos_adaptive                               = try(each.value.interface.adaptive_qos, false)
  qos_adaptive_bandwidth_downstream          = try(each.value.interface.adaptive_qos_shaping_rate_downstream != null, null)
  qos_adaptive_bandwidth_upstream            = try(each.value.interface.adaptive_qos_shaping_rate_upstream != null, null)
  qos_adaptive_default_downstream            = try(each.value.interface.adaptive_qos_shaping_rate_downstream.default, null)
  ...
  qos_shaping_rate                           = try(each.value.interface.shaping_rate, null)
  qos_shaping_rate_variable                  = try("{{${each.value.interface.shaping_rate_variable}}}", null)
  security_group_tag                         = try(each.value.interface.trustsec.security_group_tag, null)
  security_group_tag_variable                = try("{{${each.value.interface.trustsec.security_group_tag_variable}}}", null)
  service_provider                           = try(each.value.interface.service_provider, null)
  service_provider_variable                  = try("{{${each.value.interface.service_provider_variable}}}", null)
  shutdown                                   = try(each.value.interface.shutdown, null)
  shutdown_variable                          = try("{{${each.value.interface.shutdown_variable}}}", null)
  ...
  tloc_extension                    = try(each.value.interface.tloc_extension, null)
  tloc_extension_variable           = try("{{${each.value.interface.tloc_extension_variable}}}", null)
  trusted                           = try(each.value.interface.trustsec.trusted, null)
