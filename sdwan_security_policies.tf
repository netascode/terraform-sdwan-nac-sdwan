resource "sdwan_zone_based_firewall_policy_definition" "zone_based_firewall_policy_definition" {
  ...
  mode           = try(each.value.mode, local.defaults.security_policies.definitions.zone_based_firewall.mode)
  rules = [for r in each.value.rules : {
    rule_order  = r.id
    rule_name   = r.name
    base_action = r.base_action
    action_entries = (
      try(r.actions.log, null) == true ? (try(each.value.mode, local.defaults.security_policies.definitions.zone_based_firewall.mode) == "unified" ? (r.base_action == "inspect" ? [{ type = "connectionEvents" }] : [{ type = "log" }]) : [{ type = "log" }]
    ) : null)
    match_entries = (
      ...
      flatten([
        ...
        try(r.match_criterias.local_application_list, null) == null ? [] : [{
          type      = try(each.value.mode, local.defaults.security_policies.definitions.zone_based_firewall.mode) == "security" ? "appList" : "appListFlat"
  ...
  apply_zone_pairs = try(each.value.mode, local.defaults.security_policies.definitions.zone_based_firewall.mode) == "security" ? [for zp in each.value.zone_pairs : {
...
resource "sdwan_security_policy" "security_policy" {
  ...
  name        = each.value.name
  description = each.value.description
  mode        = try(each.value.mode, local.defaults.security_policies.feature_policies.mode)
  use_case    = each.value.use_case
  definitions = flatten([
    try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "security" ?
  ...
  direct_internet_applications = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "security" ? (
  ...
  high_speed_logging_vpn         = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "security" ? try(each.value.additional_settings.firewall.high_speed_logging.vpn_id, null) : null
  high_speed_logging_server_ip   = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "security" ? try(each.value.additional_settings.firewall.high_speed_logging.server_ip, null) : null
  high_speed_logging_server_port = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "security" ? try(each.value.additional_settings.firewall.high_speed_logging.server_port, null) : null
  # high_speed_logging_server_source_interface = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "unified" ? try(each.value.additional_settings.firewall.high_speed_logging.source_interface, null) : null
  max_incomplete_icmp_limit = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "unified" ? try(each.value.additional_settings.firewall.max_incomplete_icmp_limit, null) : null
  max_incomplete_tcp_limit  = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "unified" ? try(each.value.additional_settings.firewall.max_incomplete_tcp_limit, null) : null
  max_incomplete_udp_limit  = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "unified" ? try(each.value.additional_settings.firewall.max_incomplete_udp_limit, null) : null
  audit_trail = (
  ...
  match_statistics_per_filter = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "security" ? (
  ...
  unified_logging = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "unified" ? (
  try(each.value.additional_settings.firewall.unified_logging, null)) : null
  session_reclassify_allow = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "unified" ? (
  try(each.value.additional_settings.firewall.session_reclassify_allow, null)) : null
  imcp_unreachable_allow = try(each.value.mode, local.defaults.security_policies.feature_policies.mode) == "unified" ? (
